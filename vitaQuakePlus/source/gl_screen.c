...
// the finale prints the characters one at a time
	...
	if (scr_center_lines <= 4)
		y = vid.height*(0.35/cl_textscale.value);
	else
		y = 48/cl_textscale.value;
	...
	// scan the width of the line
		for (l=0 ; l<40 ; l++)
			if (start[l] == '\n' || !start[l])
				break;
		x = (vid.width - (l*8)*cl_textscale.value)/(2*cl_textscale.value);
		for (j=0 ; j<l ; j++, x+=8)
		{
...
//
	...
	ch = loadtextureimage ("gfx/ar", 0, 0, qfalse, qfalse);
...
//
	...
	{
		if (crosshair.value)
			{//Diabolickal FIXME: Make a function for this
			GL_DisableState(GL_ALPHA_TEST);
			glEnable (GL_BLEND);				
			GL_Bind (ch);
			DrawQuad(scr_vrect.x + scr_vrect.width/2-13, scr_vrect.y + scr_vrect.height/2, 27, 27, 1, 1, 1, 1);
