...
CVAR (cl_textscale,	1,	CVAR_ARCHIVE)			//Diabolickal text scaling (not fully implemented)
...
// rotate binary objects locally
		...
		else if (ent->model->flags & EF_ROCKET)
		{
			...
			VectorCopy (ent->origin, dl->origin);
			dl->radius = 200;
			dl->color[0] = 0.8f; dl->color[1] = 0.5f; dl->color[2] = 0.0f;
			dl->die = cl.time + 0.01;
		}
...
//
	...
	Cvar_RegisterVariable (&cl_textscale);			//Diabolickal text scaling
